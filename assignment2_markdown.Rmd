...
snp.allele.dt <- gdm.dt[, !c("ID", "sex", "pheno")]

#imputing the empty fields to the mean 
for (SNP.ref in colnames(snp.allele.dt)) {
  #the mean is rounded to the nearest whole number to get a count of 0,1 or 2
  avrg.allele <- round(mean(gdm.dt[[SNP.ref]], na.rm = TRUE))
  #assigning the average to the NA fiels in that column
  gdm.dt[which(is.na(gdm.dt[[SNP.ref]])==TRUE)] <- avrg.allele
}
...
univ.glm.test <- function(x,y, order=FALSE){
  ...
  for (col in colnames(x)){
    ...
    #adding the intercept and slope to the prepared vectors
    intercept.vect <- c(intercept.vect, summary(model)$coefficients[1])
