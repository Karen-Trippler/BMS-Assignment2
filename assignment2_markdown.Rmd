...
#lasso regression which only penalizes genes
...
for (cname in colnames(diab01.dt.imputed)) {
 # using `set()` is a fast way to update large data tables
 set(diab01.dt.imputed, j = cname,
 value = impute.to.median(diab01.dt.imputed[[cname]]))
}

idx.pca <- colnames(diab01.dt.imputed[, .SD, .SDcols = sapply(diab01.dt.imputed, is.numeric) & colnames(diab01.dt.imputed) != "Y"]) 
...
xdiab01.dt <- prepare.glmnet(diab01.dt.imputed[,!"PAT"], ~ . - Y)
